...
// import { courses } from "../../13_integrating-razorpay-in-react-and-node-server-starter/courses.json";

export default function CheckoutModal({ open, onClose, course, onCheckout }) {
  const [name, setName] = useState("Dushyant Choudhary");
  const [mobile, setMobile] = useState("9244526432");
  ...
  const handleProceed = async () => {
    ...
    if (!response.ok || !data.orderId) {
      throw new Error(data.error || "Order ID not received");
    }
    console.log(data.orderId);
    openRazorpayPopup({ orderId: data.orderId, course: course, user, onClose });
  };
  if (!open) return null;
...
function openRazorpayPopup({ orderId, user, course, onClose }) {
  const rzp = new window.Razorpay({
    key: "rzp_test_TXxnaCsSzv2IsO",
    currency: "INR",
    name: " Dushyant_Power_Tools",
    description: "payment",
    order_id: orderId,
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR-7-eZblv62EG9ovDcr1RNeSLK7GKwIMonRdteMgBwmA&s",
    theme: {
      color: "rgba(30, 184, 235, 0.13)",
    },
    prefill: {
      name: user.name,
      email: user.email,
